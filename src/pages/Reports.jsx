...
const generateReceiptPDF = (saleData, showNotification) => {
  try {
    ...
    doc.text(formatRupiah(total), pageWidth - margin - 5, y, { align: "right" }); y += 5;

    doc.text(`${saleData.paymentMethod === "QRIS" ? "QRIS MIDTRANS" : "TUNAI"} :`, margin, y);
    doc.text(formatRupiah(cash), pageWidth - margin - 5, y, { align: "right" }); y += 5;

    doc.text("KEMBALI :", margin, y);
    doc.text(formatRupiah(change), pageWidth - margin - 5, y, { align: "right" }); y += 10;
